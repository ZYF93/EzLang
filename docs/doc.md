...
* 向量：`Vec<Type>[N]`（SIMD 向量），其中 N 为 2, 4, 8, 16，Type 为数值类型
...
// 泛型变量（类型推断）
let list: Vec<T> = Vec<I32>[]  // T 推断为 I32
```
...
* 向量字面量：`Vec[1, 2, 3, 4]`，根据上下文推断类型或指定类型参数。
* 向量操作：支持算术运算、位运算、比较运算的向量版本。

### 12.2 语义说明
* SIMD 操作并行应用于向量元素。
* 向量长度必须匹配，否则编译错误。
* 支持向量与标量的混合操作，标量广播到向量。

#### 示例
```ez
let v1: Vec<I32>[4] = Vec[1, 2, 3, 4]
let v2: Vec<I32>[4] = Vec[5, 6, 7, 8]
let sum = v1 + v2  // Vec<I32>[4](6, 8, 10, 12)
let scaled = v1 * 2  // Vec<I32>[4](2, 4, 6, 8)
