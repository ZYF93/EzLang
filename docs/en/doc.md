...
- **Arena memory model**:
  ...
  - Capturing closures are an exception: closure capture slots and environments use compiler-generated reference-counted heap objects so returned closures, global/export variables, and fields can safely retain them. Closure-slot overwrites, local-scope exit, and return-value ownership transfer insert retain/release automatically.
...
- Anonymous functions can capture local variables used from the current scope and produce closure values. Capture slots preserve shared-variable semantics, so the closure and the outer scope read and write the same captured variable. Escaping closure capture slots and environments are managed by a reference-counted heap, so closures can be returned or stored in globals/exported variables or fields; the final closure release reclaims the captured environment.
...
- Function values produce closure structs `{ invoke, env }`. Ordinary function closures may have a null `env`. Capturing anonymous functions use a reference-counted heap environment; environment destruction releases the capture slots it owns. Calls load `invoke` and `env` before executing.
...
- **Current implementation**: the compiler records Flow/parallel/suspend-point metadata and emits linkable hooks such as `__ezrt_flow_*`, `__ezrt_parallel_*`, `__ezrt_sleep`, `__ezrt_race_i32`, `__ezrt_race_value`, `__ezrt_task_start`, and `__ezrt_task_join` in LLVM IR. `__ezrt_sleep` actually suspends the current execution point. `race(pl = [...], timeout = ...)` runs branches concurrently through the C task runtime on native targets and through `packages/std/emcc/runtime.js` + Asyncify on emcc. Zero-capture `() => I32` race branches keep a compact `__ezrt_race_i32` fast path; captured or non-`I32` branches use an out-pointer result slot. `return` inside `flow` / `parallel` blocks is captured as the expression result and does not return early from the outer function. `return` inside nested control flow also participates in expression return-type inference.
- **Blocking operations**: outside Flow, calls such as `fetch()` feel synchronous to the user. Inside Flow, `sleep`, `race(pl)`, `parallel`, and on emcc `fetch`, TCP/UDP, WebSocket, stdin, file system, process, and stream I/O can act as suspend sources that suspend and resume. Platforms without a capability return documented failure values or continue using blocking syscalls; they must not busy-wait on the CPU.
- **parallel blocks**: `const ret = parallel { code... return... }` or `const ret: T = parallel { code... return... }` starts a background task when inside Flow and the initializer expression itself is a `parallel` block. Reading `ret` waits for task completion, and exiting Flow joins unread tasks to ensure side effects are committed. Return values use caller-provided out storage, so primitive, pointer, closure, and aggregate results are covered. Captured outer locals are promoted to Flow-protected shared storage slots so both sides read and write the same variable; these slots and task environments live until pending futures have joined and are then reclaimed with the Flow-protected Arena scope. Locked variables still use the existing `rp` / `wp` / default ordered lock hooks. Native, Android, and iOS targets use the C task runtime; emcc uses the JS coroutine runtime. Compound expressions still use synchronous cooperative lowering.
- **Automatic dependency waiting**: reading an unfinished `parallel` result inside Flow automatically joins it. Current dependency waiting covers native, Android, iOS, and emcc tasks, including inferred types, explicit declarations, non-`I32` return values, aggregate return values, and shared-capture task environments. emcc standard-library suspend sources restore the wasm stack through Asyncify and preserve sequential ABI.
...
- **race function**: native, Android, iOS, and emcc targets run `pl` branches concurrently, return the first completed value, and cancel or ignore remaining branches. Timeout returns a zero value and marks the timeout slot. Zero-capture `() => I32` branches use the compact I32 race ABI; captured branches and other return types use caller-Arena result slots, branch scratch storage, and an out-pointer race ABI. The runtime completes or cancels and joins branches before returning. Exceptions are returned through the runtime exception slot to the outer `catch`.
...
- **Current**: lowering uses runtime hooks plus Flow/parallel return slots. `sleep`, `race(pl)`, and `parallel` inside Flow are connected to the native task runtime; Android/iOS reuse the same C runtime; emcc uses the Asyncify JS coroutine runtime for suspendable behavior. emcc standard-library `sleep`, HTTP `fetch`, TCP/UDP, WebSocket `wsConnect` / `wsRecv`, stdin, fs, process, and stream I/O carry Asyncify metadata. The CLI automatically adds `-sASYNCIFY`. Future backends can replace the emcc runtime with JSPI or wasm pthreads without changing EzLang syntax.
- **Goal**: native blocking I/O can later be replaced by state machines, platform wait sources such as epoll, io_uring, kqueue, and timerfd, result storage, and wakeups.
