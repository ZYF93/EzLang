...
4. **Flow 优先**：阻塞 I/O 以 `flow` suspend point 为语义目标；当前已接入的调度点包括 `sleep`、`race(pl)`、部分 `parallel` 任务，以及 emcc 下的 HTTP、stdin、fs、process 和 stream I/O。
...
| `std/net/http` | 原生/JS 封装 | 明文 HTTP 客户端；基础服务端 | 明文 HTTP 客户端；基础服务端 | 显式失败 | fetch + Asyncify 客户端；无 Asyncify 时同步 XHR fallback；服务端不支持 | 不支持 |
...
| `std/process` | 原生/JS 封装 | 子进程 exec/spawn/wait/管道流 | 子进程 exec/spawn/wait/管道流 | 显式失败 | Node spawn + Asyncify；stdout/stderr 可转流；浏览器显式失败 | 不支持 |
...
| `platformHasSubprocess`   | Node emcc 有 `child_process.spawn` / `spawnSync` 时返回 `true`；浏览器 WebAssembly、iOS、Android 返回 `false` |
...
当前 Linux/macOS/Windows native 目标实现子进程调用和 stdin/stdout/stderr 管道流转交；emcc 的 Node 风格运行时在 Asyncify 可用时使用 `child_process.spawn` 挂起等待，缺少 Asyncify 时回退 `child_process.spawnSync`，`processStdout` / `processStderr` 可把已完成结果转成可读 `Stream`，`processStdin` 因当前 emcc 结果句柄没有活管道而返回空可选值。浏览器 WebAssembly 仍返回空可选值或 `false`，不伪装为成功。emcc 的 `processSpawn` 保存已完成的结果，`processWait` 读取该结果；若 stdout/stderr 已转交为流，`processWait` 中对应 Blob 为空。Android、iOS 目标当前仍显式失败。
...
当前 `std/net/http` 在原生桌面平台支持明文 `http://` 客户端请求，并可发送/读取基础字符串请求头和解码 chunked 响应体；客户端 URL 支持合法端口、userinfo 剥离、根 query 保留、fragment 剥离和 IPv6 字面量，并会按 authority 生成正确的 `Host` 头。原生桌面平台支持基础阻塞式 HTTP 服务端，可注册精确路径路由并返回状态码、响应头和响应体；emcc 在浏览器/Worker 环境优先使用 `fetch` + Asyncify 挂起客户端请求，无 Asyncify 时保留同步 `XMLHttpRequest` fallback，服务端仍明确不支持。`std/net/tcp` / `std/net/ws` 可创建原生 socket 句柄，并提供基础阻塞式 TCP/UDP 读写收发；WebSocket 原生 `ws://` 客户端支持合法端口、userinfo 剥离、IPv6 字面量、客户端掩码、分片帧重组和 ping/pong 自动处理。HTTPS、WebSocket `wss://`、HTTP 服务端并发 worker 和 native 事件源式 flow 挂起仍待补齐。不支持的网络入口会显式返回空可选值或零句柄，不会伪装成请求或连接成功。

`HttpResponse.body` 始终保留原始字节；`HttpResponse.text()` 仅在响应体是不含空字节的合法 UTF-8 时返回文本，否则返回空字符串。

后续接入 TLS、超时配置和 native 事件源式 flow 挂起时保留现有签名。当前网络 API 在 `flow` 外为同步阻塞；emcc HTTP 客户端在 Asyncify 可用时会挂起后恢复，原生 TCP/UDP/WS 仍使用阻塞 socket 调用。
...
| emcc                    | 浏览器/Worker `fetch` + Asyncify 支持客户端请求；无 Asyncify 时回退同步 XHR；两者都不可用时返回空可选值 |
...
当前原生桌面平台的 TCP API 会创建真实阻塞 socket 句柄，支持连接、监听、接收连接、基础读写和关闭。端口必须在 `0..65535` 范围内，非法端口会返回空可选值。Android/iOS 复用同一原生封装入口；emcc 当前仍返回空可选值或失败值，不会同步等待网络。native 事件源式 flow 挂起、超时配置、TLS 与跨平台细节仍需后续运行时接入。
...
当前原生桌面平台的 UDP API 会创建真实阻塞 UDP socket 句柄，支持绑定、向指定地址发送、接收数据、接收数据及来源地址和关闭。端口必须在 `0..65535` 范围内；非法绑定端口会返回空可选值，非法发送端口会返回 `-1`。Android/iOS 复用同一原生封装入口；emcc 当前仍返回空可选值或失败值，不会同步等待网络。超时和 native 事件源式 flow 挂起仍需后续运行时接入。
