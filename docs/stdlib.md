...
EzLang 标准库采用"统一上层 API，平台感知底层实现"的设计策略。编译器根据 `project.toml` 中定义的目标平台（target）动态切换底层链接库与系统调用。阻塞 I/O 遵循 Flow 并发语义目标：在 `flow {}` 内可作为 suspend point 调度，在 `flow` 外等价于同步阻塞；当前未接入调度器的平台封装保持同步 ABI，并在平台说明中标注限制。
...
4. **Flow 优先**：阻塞 I/O 以 `flow` suspend point 为语义目标；当前已接入的调度点包括 `sleep`、`race(pl)` 和部分 `parallel` 任务，其它封装保持同步 ABI。
...
| `std/io` | 原生/JS 封装 | stdout/stderr/stdin | stdout/stderr/stdin | 系统日志；stdin 空可选值 | console；Node stdin；浏览器 stdin 空可选值 | 不支持 |
...
| `std/net/http` | 原生/JS 封装 | 明文 HTTP 客户端；基础服务端 | 明文 HTTP 客户端；基础服务端 | 显式失败 | 同步 XHR 客户端；服务端不支持 | 不支持 |
| `std/net/tcp` | 原生/JS 封装 | TCP/UDP 阻塞式 socket | TCP/UDP 阻塞式 socket | TCP/UDP 阻塞式 socket | 显式失败 | 不支持 |
| `std/net/ws` | 原生/JS 封装 | ws 客户端/分片/ping-pong | ws 客户端/分片/ping-pong | ws 客户端/分片/ping-pong | 显式失败 | 不支持 |
...
| `std/process` | 原生/JS 封装 | 子进程 exec/spawn/wait/管道流 | 子进程 exec/spawn/wait/管道流 | 显式失败 | Node 同步子进程；stdout/stderr 可转流；浏览器显式失败 | 不支持 |
| `std/uri` | 纯逻辑封装 | URL 解析/构造/编码 | URL 解析/构造/编码 | URL 解析/构造/编码 | URL 解析/构造/编码 | 不支持 |
| `std/debug` | 原生/JS 封装 | 诊断/堆栈尽力捕获 | 诊断/堆栈尽力捕获 | 诊断可用；移动端堆栈显式失败 | console/Error stack | 不支持 |
| `std/log` | std/io + std/time | stdout/stderr/file | stdout/stderr/file | 系统日志入口/file | console；Emscripten FS/Node 文件目标 | 不支持 |
| `std/stream` | 运行时 ABI 封装 | 内存/Blob、文件流、TCP 流、进程管道流 | 内存/Blob、文件流、TCP 流、进程管道流 | 内存/Blob、文件流、TCP 流、进程管道流 | 内存/Blob、文件流、进程 stdout/stderr 流 | 不支持 |
...
| `std/compress` | 平台压缩库封装 | zlib/gzip/deflate | zlib/gzip/deflate | Android/iOS zlib/gzip/deflate | Node zlib；浏览器同步 ABI 显式失败 | 不支持 |
...
| `readLine` | 从 stdin 读取一行，返回 `Str?` | 空可选值             | Node 读取 fd 0；浏览器空可选值    |
...
| `platformHasSubprocess`   | Node emcc 有同步 `child_process.spawnSync` 时返回 `true`；浏览器 WebAssembly、iOS、Android 返回 `false` |
...
当前 Linux/macOS/Windows native 目标实现子进程调用和 stdin/stdout/stderr 管道流转交；emcc 的 Node 风格同步运行时使用 `child_process.spawnSync` 实现执行与输出捕获，`processStdout` / `processStderr` 可把已完成结果转成可读 `Stream`，`processStdin` 因同步结果句柄没有活管道而返回空可选值。浏览器 WebAssembly 仍返回空可选值或 `false`，不伪装为成功。emcc 的 `processSpawn` 在同步 ABI 下保存已完成的 `spawnSync` 结果，`processWait` 读取该结果；若 stdout/stderr 已转交为流，`processWait` 中对应 Blob 为空。Android、iOS 目标当前仍显式失败。`flow` 调度状态机完成前，进程等待在运行时仍是同步阻塞调用。
...
`.` 默认不匹配换行符。该模块不是完整 PCRE 兼容实现。Windows 内置 fallback 支持字面量、`.`、`^`/`$`、分组、`|`、字符类/范围、常用 POSIX 字符类、`?`/`*`/`+` 量词、捕获组、全局查找/替换和分割；复杂 Unicode 类、回溯控制、环视断言、命名捕获等语法是否可用取决于底层平台；高危正则的复杂度限制仍需后续运行时超时/取消机制配合。
...
native 目标使用系统 zlib；对应工具链需要提供 zlib 开发库。WebAssembly 同步 ABI 下优先使用 Node `zlib`，浏览器同步环境当前没有同步压缩 API，一次性 API 返回空可选值，流式 API 返回 `-1`。
...
当前 `std/net/http` 在原生桌面平台支持明文 `http://` 客户端请求，并可发送/读取基础字符串请求头和解码 chunked 响应体；客户端 URL 支持合法端口、userinfo 剥离、根 query 保留、fragment 剥离和 IPv6 字面量，并会按 authority 生成正确的 `Host` 头。原生桌面平台支持基础阻塞式 HTTP 服务端，可注册精确路径路由并返回状态码、响应头和响应体；emcc 在浏览器/Worker 环境使用同步 `XMLHttpRequest` 支持客户端请求，服务端仍明确不支持。`std/net/tcp` / `std/net/ws` 可创建原生 socket 句柄，并提供基础阻塞式 TCP/UDP 读写收发；WebSocket 原生 `ws://` 客户端支持合法端口、userinfo 剥离、IPv6 字面量、客户端掩码、分片帧重组和 ping/pong 自动处理。HTTPS、WebSocket `wss://`、HTTP 服务端并发 worker 和异步 flow 挂起仍待补齐。不支持的网络入口会显式返回空可选值或零句柄，不会伪装成请求或连接成功。

`HttpResponse.body` 始终保留原始字节；`HttpResponse.text()` 仅在响应体是不含空字节的合法 UTF-8 时返回文本，否则返回空字符串。

后续接入 TLS、超时配置和完整异步 flow 挂起时保留现有签名。当前网络 API 在 `flow` 外为同步阻塞；在完整调度接入前，`flow {}` 内也沿用同步 ABI，不伪装为已异步化。
...
| 目标                    | 行为                                      |
| ----------------------- | ----------------------------------------- |
| linux / macos / windows | `fetch` / `fetchEx` 支持明文 `http://`、合法端口、userinfo、IPv6 字面量、基础请求/响应头和 chunked 响应体；HTTPS 和解析失败返回空可选值 |
| android / ios           | `fetch` / `fetchEx` 返回空可选值          |
| emcc                    | 浏览器/Worker 同步 XHR 支持客户端请求；无同步 XHR 时返回空可选值 |

---

### 6.2 HTTP 服务端

原生桌面平台的 HTTP 服务端为基础阻塞式实现：`createServer` 创建服务端句柄，`on` 注册精确路径路由，`start` 在当前线程监听并处理连接，`stop` 关闭监听句柄。emcc 目标仍不支持服务端，`createServer` 返回空句柄。
...
当前原生桌面平台的 TCP API 会创建真实阻塞 socket 句柄，支持连接、监听、接收连接、基础读写和关闭。端口必须在 `0..65535` 范围内，非法端口会返回空可选值。Android/iOS 复用同一原生封装入口；emcc 当前仍返回空可选值或失败值。异步 flow 挂起、超时配置、TLS 与跨平台细节仍需后续运行时接入。
...
当前原生桌面平台的 UDP API 会创建真实阻塞 UDP socket 句柄，支持绑定、向指定地址发送、接收数据、接收数据及来源地址和关闭。端口必须在 `0..65535` 范围内；非法绑定端口会返回空可选值，非法发送端口会返回 `-1`。Android/iOS 复用同一原生封装入口；emcc 当前仍返回空可选值或失败值。超时和异步 flow 挂起仍需后续运行时接入。
...
当前原生桌面平台的 `wsConnect` 支持 `ws://` 握手并返回 socket 句柄，会校验状态码、`Upgrade`、`Connection` 和 `Sec-WebSocket-Accept`；URL 支持合法端口、userinfo 剥离、根 query 保留、fragment 剥离和 IPv6 字面量，并会按 authority 生成正确的 `Host` 头。`wsSend` 使用客户端掩码发送二进制消息，`wsRecv` 支持文本/二进制消息、分片帧重组并自动处理 ping/pong 控制帧，`wsClose` 发送关闭帧后关闭 socket。Android/iOS 复用同一原生封装入口；emcc 当前仍返回空可选值或失败值。`wss://`、浏览器 WebSocket 桥接和异步 flow 挂起仍需后续接入。
...
2. **Flow 优先**：阻塞 I/O 以 `flow` suspend point 为语义目标；在 `flow` 外等价于同步阻塞。语言不引入 `async/await`，并发语义统一由 `flow` / `parallel` / `race` 表达；未接入调度器的封装保持同步 ABI。
