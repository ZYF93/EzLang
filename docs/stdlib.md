...
| `std/collections` | compiler lowering | 完整 | 完整 | 完整 | 完整 | 不支持 |
...
能力类型说明：`compiler builtin` 由编译器内建提供，不依赖标准库声明或外部符号；`compiler lowering` 通过标准库公开 API 声明，由编译器 lowering，不依赖外部符号；`纯逻辑封装` 不访问系统资源；`原生/JS 封装` 通过 C 或 Emscripten JS ABI 访问平台能力；`运行时 ABI 封装` 固定语言运行时数据布局，后续可接入文件、网络、进程等具体后端。
...
以下类型由编译器预声明，用户代码可以直接使用，不需要 `from "std/..." import`：
...
- `Date`、`Error`、`Blob` 是跨标准库模块共享的 ABI 类型。`std/time`、`std/mem`、`std/fs`、`std/net/*`、`std/stream` 等模块会直接使用这些类型。
- `List<T>` / `T[]` 和 `Dict<K, V>` 也是编译器内置复合类型；标准库的 `std/collections` 只提供操作函数，不负责定义底层类型。
- `Meta<T>` 供装饰器使用。`@Dec let value = ...` 会把顶层变量包装为 `Meta<T>`，装饰器可设置 `getter` / `setter` 闭包拦截读写；`t` 和 `name` 分别暴露编译器生成的类型名和变量名。
- 这些内置类型的完整语义见 `docs/doc.md` 的“结构体、组合与内置对象”和“元编程、语法糖与安全机制”。

---

## 0.2 外部 ABI 链接 (`extern`)
...
用户代码可直接 `import` 标准库 API，编译器根据目标平台自动选择正确的 `extern` 库和 `declare` 声明；底层 C/JS 符号不是额外公开的 Ez API。
...
`Command.env` 使用 `KEY=VALUE` 字符串数组表示环境变量覆盖，保持与 POSIX/Windows/Node 进程环境 ABI 一致，避免跨平台封装层额外转换字典。`Command.stdin` 必须是合法 `Blob`；非法 `Blob` 输入会让 `processExec` / `processSpawn` 返回空可选值。`ProcessResult.ok` 表示退出码是否为 `0`。
...
查询参数接口使用字符串级 API，保留重复键、空键、顺序和原始编码边界；需要结构化访问时可由用户在上层转换为 `Dict`。HTTP 模块可复用 `UriParts` 和这些编解码规则。
...
`std/log` 提供编译期/运行时级别过滤、统一日志格式、代码位置和键值字段输出。字段使用 `Str[]` 表示为偶数位 `key, value` 序列，便于 native/JS 封装顺序输出，并允许重复键。
...
```ez
from "std/collections" import { listPush, listLen }
