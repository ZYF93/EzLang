...
- **Flow 并发运行时**：内置 `flow {}` / `parallel {}` / `race(pl)` 并发模型，native 目标已支持零捕获 `I32` 任务运行时，emcc 使用同步协作 fallback 保持语法可用。
...
- 阻塞 I/O 以 `flow {}` suspend point 为语义目标；当前未接入调度器的平台封装保持同步 ABI
...
# 可选：使用镜像仓库
EZLANG_REPO_URL=https://github.com/ZYF93/EzLang.git sh install.sh

# 兼容：也可以直接传入仓库地址
sh install.sh https://github.com/ZYF93/EzLang.git
```

安装脚本默认从 `https://github.com/ZYF93/EzLang.git` 拉取或更新源码，准备虚拟环境，安装 CLI 与编译器，执行一次最小本机编译校验，并把 `~/.ezlang/bin` 注册到 shell 配置文件。

基础安装需要 Python 3.9+、`git` 和本机 C 编译器 `cc`。Python 包依赖（包括 `llvmlite` 提供的 LLVM binding）由脚本通过 `pip` 安装，不要求用户单独安装系统 LLVM。`emcc` 不是基础安装硬依赖，只在构建 `os = "emcc"` / `wasm32` 目标时需要 Emscripten SDK。
